#!/usr/bin/env python3
"""
Train plate detection model (config-driven).
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "config" / "training" / "plate_detection_train.yaml",
    )
    ...
    if not data_path.exists():
        raise FileNotFoundError(
            f"Plate data file missing: {data_path}. Run: python scripts/prepare_dataset.py"
        )
